...
DETAILED_LOG_FILE = "clone_detailed_log.txt"
...
# Create detailed logger for real-time message tracking
...
class TelegramCloner:
    """Advanced Telegram Group Cloner v7.1 - Enhanced Logging Edition"""
    ...
    def log_message_details(self, message: Message, action: str, status: str, 
                           target_id: Optional[int] = None, media_position: Optional[int] = None,
                           additional_info: str = ""):
        """Log detailed message information in real-time"""
        ...
        log_entry = {
            ...
            "caption_preview": message.caption[:100] if message.caption else None,
            "additional_info": additional_info
        }
        
        # Add media-specific details
        if message.photo:
            log_entry["file_id"] = message.photo.file_id
            log_entry["file_unique_id"] = message.photo.file_unique_id
            log_entry["file_size"] = message.photo.file_size
        elif message.video:
            log_entry["file_id"] = message.video.file_id
            log_entry["file_unique_id"] = message.video.file_unique_id
            log_entry["file_size"] = message.video.file_size
            log_entry["duration"] = message.video.duration
            log_entry["resolution"] = f"{message.video.width}x{message.video.height}"
        elif message.document:
            log_entry["file_id"] = message.document.file_id
            log_entry["file_unique_id"] = message.document.file_unique_id
            log_entry["file_size"] = message.document.file_size
            ...
            log_entry["file_size"] = message.audio.file_size
            log_entry["duration"] = message.audio.duration
            log_entry["title"] = message.audio.title
            log_entry["performer"] = message.audio.performer
        
        # Format for human-readable log
        media_pos_str = f" | Media #{media_position}/{self.total_media_count}" if media_position else ""
        text_preview = f" | Text: '{log_entry['text_preview']}...'" if log_entry['text_preview'] else ""
        caption_preview = f" | Caption: '{log_entry['caption_preview']}...'" if log_entry['caption_preview'] else ""
        target_str = f" → Target ID: {target_id}" if target_id else ""
        
        log_line = (f"[{status.upper()}] {action} | Type: {msg_type} | "
                   f"Source ID: {message.id}{media_pos_str}{target_str}{text_preview}{caption_preview}")
        
        if additional_info:
            log_line += f" | {additional_info}"
        
        # Write to detailed log
        detailed_logger.info(log_line)
        detailed_logger.info(f"    Full Details: {json.dumps(log_entry, ensure_ascii=False)}")
        detailed_logger.info("-" * 100)
    
    def log_media_group_details(self, messages: List[Message], action: str, status: str,
                               target_ids: Optional[List[int]] = None, additional_info: str = ""):
        """Log detailed media group information"""
        group_id = messages[0].media_group_id
        
        media_positions = [self.media_position_map.get(msg.id) for msg in messages]
        media_pos_str = f"{min(media_positions)}-{max(media_positions)}/{self.total_media_count}" if all(media_positions) else "N/A"
        ...
        log_entry = {
            ...
            "media_positions": media_pos_str,
            "message_types": [self.get_message_type(msg) for msg in messages],
            "first_date": messages[0].date.isoformat() if messages[0].date else None,
            "has_captions": [bool(msg.caption) for msg in messages],
            "additional_info": additional_info
        }
        
        target_str = f" → Target IDs: {target_ids}" if target_ids else ""
        
        log_line = (f"[{status.upper()}] {action} | Media Group: {group_id} | "
                   f"Items: {len(messages)} | Source IDs: {messages[0].id}-{messages[-1].id} | "
                   f"Media Position: {media_pos_str}{target_str}")
        ...
        for i, msg in enumerate(messages, 1):
            ...
            media_pos = self.media_position_map.get(msg.id, "N/A")
            caption = f" Caption: '{msg.caption[:50]}...'" if msg.caption else ""
            detailed_logger.info(f"    Item {i}/{len(messages)}: ID {msg.id} | Type: {msg_type} | Media #{media_pos}{caption}")
    ...
    async def scan_target_group(self) -> Set[str]:
        ...
        detailed_logger.info("="*100)
        detailed_logger.info("STARTING TARGET GROUP SCAN")
        detailed_logger.info("="*100)
        ...
        try:
            async for message in self.app.get_chat_history(self.target):
                ...
                if count % 500 == 0:
                    ...
                    detailed_logger.info(f"Scanned {count} messages in target group")
            ...
            detailed_logger.info(f"Target scan complete: {len(target_hashes)} unique messages found")
            detailed_logger.info("="*100 + "\n")
            
        except Exception as e:
            logger.error(f"Error scanning target group: {e}")
            detailed_logger.error(f"Error during target scan: {e}")
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if fresh and not fresh.empty:
                    detailed_logger.info(f"Successfully refetched message {message_id} on attempt {attempt + 1}")
                    return fresh
            except FloodWait as e:
                logger.warning(f"Flood wait while refetching {message_id}: {e.value}s")
                detailed_logger.warning(f"Flood wait during refetch of {message_id}: {e.value}s")
                await asyncio.sleep(e.value)
            except Exception as e:
                logger.warning(f"Attempt {attempt+1}/{max_attempts} to refetch {message_id} failed: {e}")
                detailed_logger.warning(f"Refetch attempt {attempt+1}/{max_attempts} failed for {message_id}: {e}")
                if attempt < max_attempts - 1:
                    await asyncio.sleep(2 ** attempt)
        
        logger.error(f"Could not fetch fresh message {message_id} after {max_attempts} attempts")
        detailed_logger.error(f"Failed to refetch message {message_id} after {max_attempts} attempts")
        return None
    ...
    async def get_fresh_messages_bulk(self, message_ids: List[int]) -> List[Optional[Message]]:
        ...
        try:
            ...
            detailed_logger.info(f"Bulk refetched {len(message_ids)} messages")
            ...
            detailed_logger.warning(f"Flood wait during bulk refetch: {e.value}s")
            await asyncio.sleep(e.value)
            return await self.get_fresh_messages_bulk(message_ids)
            
        except Exception as e:
            logger.error(f"Error bulk refetching: {e}")
            detailed_logger.error(f"Bulk refetch failed: {e}, falling back to individual fetches")
            results = []
    ...
    async def clone_media_group(self, messages: List[Message], retry_count: int = 0) -> Tuple[str, str, float]:
        ...
        if str(media_group_id) in self.progress.get("failed_groups", {}):
            ...
            if fail_count >= MAX_RETRIES:
                ...
                self.log_media_group_details(messages, "CLONE_MEDIA_GROUP", "SKIPPED", 
                                            additional_info=f"Failed {fail_count} times, max retries reached")
                return "failed", "media_group", INITIAL_DELAY
        ...
        try:
            ...
            if not media_list:
                self.log_media_group_details(messages, "CLONE_MEDIA_GROUP", "FAILED", 
                                            additional_info="No valid media objects")
                return "failed", "media_group", INITIAL_DELAY
            ...
            for orig_msg, sent_msg in zip(messages, sent_messages):
                ...
                target_ids.append(sent_msg.id)
            
            logger.info(f"✅ Cloned media group {media_group_id} with {len(media_list)} items")
            self.log_media_group_details(messages, "CLONE_MEDIA_GROUP", "SUCCESS", 
                                        target_ids=target_ids,
                                        additional_info=f"Retry attempt: {retry_count}")
            ...
            if retry_count < MAX_RETRIES:
                ...
                self.log_media_group_details(messages, "CLONE_MEDIA_GROUP", "RETRY", 
                                            additional_info=f"FileReferenceExpired, retry {retry_count + 1}/{MAX_RETRIES}")
                ...
                if len(fresh_messages) == len(messages):
                    ...
                    self.log_media_group_details(messages, "CLONE_MEDIA_GROUP", "FAILED", 
                                                additional_info="Could not refetch all messages")
                ...
                self.log_media_group_details(messages, "CLONE_MEDIA_GROUP", "FAILED", 
                                            additional_info=f"Max retries ({MAX_RETRIES}) reached")
                if "failed_groups" not in self.progress:
                    self.progress["failed_groups"] = {}
                self.progress["failed_groups"][str(media_group_id)] = retry_count + 1
                return "failed", "media_group", INITIAL_DELAY
            ...
            self.log_media_group_details(messages, "CLONE_MEDIA_GROUP", "RETRY", 
                                        additional_info=f"FloodWait {e.value}s")
            await asyncio.sleep(e.value)
            ...
            if "FILE_REFERENCE" in error_msg and retry_count < MAX_RETRIES:
                ...
                self.log_media_group_details(messages, "CLONE_MEDIA_GROUP", "RETRY", 
                                            additional_info=f"BadRequest FILE_REFERENCE, retry {retry_count + 1}/{MAX_RETRIES}")
                ...
                if len(fresh_messages) == len(messages):
                    ...
                    self.progress["failed_groups"][str(media_group_id)] = retry_count + 1
                    self.log_media_group_details(messages, "CLONE_MEDIA_GROUP", "FAILED", 
                                                additional_info="Could not refetch all messages after BadRequest")
                    return "failed", "media_group", INITIAL_DELAY
            else:
                logger.error(f"Bad request for group {media_group_id}: {e}")
                self.log_media_group_details(messages, "CLONE_MEDIA_GROUP", "FAILED", 
                                            additional_info=f"BadRequest: {e}")
                if "failed_groups" not in self.progress:
                    self.progress["failed_groups"] = {}
                self.progress["failed_groups"][str(media_group_id)] = retry_count + 1
                return "failed", "media_group", INITIAL_DELAY
    ...
    async def clone_message(self, message: Message, current_delay: float, retry_count: int = 0) -> Tuple[str, str, float]:
        ...
        media_position = self.media_position_map.get(message.id)
        ...
        try:
            ...
            # Try forwarding first if it's a forwarded message
            if message.forward_date or message.forward_from or message.forward_from_chat:
                try:
                    ...
                    if cloned_message:
                        ...
                        self.log_message_details(message, "FORWARD_MESSAGE", "SUCCESS", 
                                                cloned_message.id, media_position,
                                                f"Forwarded from original source")
                    ...
                    detailed_logger.warning(f"Forward failed for {message.id}: {fwd_error}, using clone method")
            
            # Clone based on message type
            if message_type == "text":
                ...
                self.log_message_details(message, "CLONE_MESSAGE", "SKIPPED", 
                                        None, media_position,
            ...
            if cloned_message:
                self.message_mapping[str(message.id)] = cloned_message.id
                self.log_message_details(message, "CLONE_MESSAGE", "SUCCESS", 
                                        cloned_message.id, media_position,
            ...
            if retry_count < MAX_RETRIES:
                ...
                self.log_message_details(message, "CLONE_MESSAGE", "RETRY", 
                                        None, media_position,
                                        f"FileReferenceExpired, retry {retry_count + 1}/{MAX_RETRIES}")
                ...
                if fresh_message:
                    ...
                    self.log_message_details(message, "CLONE_MESSAGE", "FAILED", 
                                            None, media_position,
                                            "Could not refetch message after FileReferenceExpired")
            ...
            if "FILE_REFERENCE" in error_msg and retry_count < MAX_RETRIES:
                ...
                self.log_message_details(message, "CLONE_MESSAGE", "RETRY", 
                                        None, media_position,
                                        f"BadRequest FILE_REFERENCE, retry {retry_count + 1}/{MAX_RETRIES}")
                await asyncio.sleep(FILE_REF_RETRY_DELAY * (retry_count + 1))
                fresh_message = await self.get_fresh_message(message.id, retry_count)
                if fresh_message:
                    await asyncio.sleep(1)
    ...
    def print_final_report(self):
        ...
        print(f"   Standard logs: clone_log.txt")
        print(f"   Detailed logs: {DETAILED_LOG_FILE}")
        ...
        detailed_logger.info("="*100)
        detailed_logger.info("CLONING OPERATION COMPLETED")
        detailed_logger.info("="*100)
        detailed_logger.info(f"Total message units: {self.stats['total_messages']}")
        detailed_logger.info(f"Total individual items: {self.stats['total_items']}")
        detailed_logger.info(f"Successfully cloned: {self.stats['successful']}")
        detailed_logger.info(f"Failed: {self.stats['failed']}")
        detailed_logger.info(f"Skipped: {self.stats['skipped']}")
        detailed_logger.info(f"Duplicates avoided: {self.stats['duplicates']}")
        detailed_logger.info(f"Total media items scanned: {self.total_media_count}")
        if self.stats['total_items'] > 0:
            overall_success = (self.stats['successful'] / self.stats['total_items'] * 100)
            detailed_logger.info(f"Overall Success Rate: {overall_success:.2f}%")
        detailed_logger.info("="*100)
    
    async def fetch_all_messages_ordered(self) -> List:
        """Fetch ALL messages in chronological order with media position tracking"""
        print("\n" + "="*70)
        print("📥 FETCHING MESSAGES IN CHRONOLOGICAL ORDER")
        print("="*70)
        
        detailed_logger.info("="*100)
        detailed_logger.info("STARTING MESSAGE FETCH - BUILDING MEDIA POSITION MAP")
        detailed_logger.info("="*100)
        ...
        try:
            # First pass: count total media and build position map
            async for message in self.app.get_chat_history(self.source):
                ...
                if (message.photo or message.video or message.document or 
                    message.audio or message.animation):
                    media_counter += 1
                    self.media_position_map[message.id] = media_counter
                
                chunk_count += 1
                
                if chunk_count % 1000 == 0:
                    print(f"📦 Scanning: {chunk_count} messages, {media_counter} media items...")
                    detailed_logger.info(f"Scan progress: {chunk_count} messages, {media_counter} media items")
                
                await asyncio.sleep(SCAN_DELAY)
            
            self.total_media_count = media_counter
            print(f"✅ Media scan complete: {self.total_media_count} total media items")
            detailed_logger.info(f"Total media items in source: {self.total_media_count}")
            detailed_logger.info(f"Media position map created with {len(self.media_position_map)} entries")
            
            # Second pass: fetch messages for cloning
            print("\n📥 Fetching messages for cloning...")
            detailed_logger.info("="*100)
            detailed_logger.info("FETCHING MESSAGES FOR CLONING")
            detailed_logger.info("="*100)
            ...
            async for message in self.app.get_chat_history(self.source):
                ...
                if msg_hash:
                    ...
                    if msg_hash in self.message_hashes["target_hashes"]:
                        ...
                        media_pos = self.media_position_map.get(message.id, "N/A")
                        detailed_logger.info(f"[DUPLICATE] Message ID: {message.id} | Media Position: {media_pos} | Type: {self.get_message_type(message)}")
                        continue
                ...
                if chunk_count % 500 == 0:
                    ...
                    detailed_logger.info(f"Fetch progress: {chunk_count} messages processed")
            ...
            for group_id, group_data in media_groups_temp.items():
                ...
                if is_duplicate:
                    ...
                    media_positions = [self.media_position_map.get(msg.id, "N/A") for msg in messages]
                    detailed_logger.info(f"[DUPLICATE] Media Group: {group_id} | {len(messages)} items | Media Positions: {media_positions}")
                    continue
            ...
            print(f"   Total media items: {self.total_media_count}")
            ...
            detailed_logger.info("="*100)
            detailed_logger.info("FETCH COMPLETE - SUMMARY")
            detailed_logger.info(f"Total scanned: {chunk_count}")
            detailed_logger.info(f"Total media items: {self.total_media_count}")
            detailed_logger.info(f"Items to clone: {len(all_items)}")
            detailed_logger.info(f"Duplicates avoided: {duplicate_count}")
            detailed_logger.info(f"Already processed: {len(already_processed)}")
            detailed_logger.info("="*100 + "\n")
            ...
            detailed_logger.error(f"Fatal error during message fetch: {e}")
    ...
    async def start_cloning(self):
        ...
        print("\n" + "="*70)
        print("🚀 TELEGRAM GROUP CLONER v7.1 - ENHANCED LOGGING EDITION")
        print("="*70)
        print(f"📍 Source: {self.source}")
        print(f"📍 Target: {self.target}")
        print("\n✨ FEATURES:")
        print("   • 100% accurate message cloning")
        print("   • Real-time detailed logging with media positions")
        print("   • Aggressive retry for file references")
        print("   • Robust media group handling")
        ...
        detailed_logger.info("="*100)
        detailed_logger.info(f"CLONING SESSION STARTED - {datetime.now().isoformat()}")
        detailed_logger.info(f"Source Group: {self.source}")
        detailed_logger.info(f"Target Group: {self.target}")
        detailed_logger.info("="*100 + "\n")
        ...
        if not all_items:
            ...
            detailed_logger.info("No items to clone - target is up to date")
        ...
        detailed_logger.info("="*100)
        detailed_logger.info(f"CLONING STARTED: {len(all_items)} message units, {total_items} total items")
        detailed_logger.info("="*100 + "\n")
        ...
        for idx, (item_id, item_type, item_data) in enumerate(all_items, 1):
            ...
            if item_type == 'group':
                ...
                if status == "success":
                    ...
                    media_positions = [self.media_position_map.get(msg.id) for msg in messages]
                    media_pos_str = f"{min(media_positions)}-{max(media_positions)}" if all(media_positions) else "N/A"
                    
                    print(f"✅ [{idx}/{len(all_items)}] Media group: {len(messages)} items (IDs: {messages[0].id}-{messages[-1].id}) | Media #{media_pos_str}/{self.total_media_count}")
                    ...
                    print(f"❌ [{idx}/{len(all_items)}] Failed media group {group_id} ({len(messages)} items) | Media #{media_pos_str}/{self.total_media_count}")
                ...
                if status == "success":
                    ...
                    print(f"✅ [{idx}/{len(all_items)}] {msg_type:20} | Message {message.id}{media_pos_str}")
                
                elif status == "failed":
                    self.progress["failed"].append(message.id)
                    print(f"❌ [{idx}/{len(all_items)}] Failed {msg_type} message {message.id}{media_pos_str}")
                
                elif status != "skipped":
                    print(f"⏭️  [{idx}/{len(all_items)}] Skipped {msg_type} message {message.id}{media_pos_str}")
            ...
            if processed_count % 10 == 0:
                ...
                detailed_logger.info(f"[PROGRESS] Processed: {processed_count}/{len(all_items)} | Success: {self.stats['successful']} | Failed: {self.stats['failed']}")
...
╔══════════════════════════════════════════════════════════════════════╗
║                                                                      ║
║   TELEGRAM GROUP CLONER v7.1 - ENHANCED LOGGING EDITION             ║
║                                                                      ║
║  ✨ NEW: Real-time detailed logging with media position tracking     ║
║  📊 Media position numbers (e.g., "510 of 200,000")                  ║
║  📝 Complete message details in clone_detailed_log.txt               ║
║  🔧 Aggressive retry logic for file references                       ║
║  📦 Robust media group handling with bulk refetch                    ║
...
║  • Source message ID + Target message ID mapping                     ║
║  • Media position numbers for easy verification                      ║
║  • Text/Caption previews in logs                                     ║
║  • File IDs and unique IDs for media                                 ║
║  • Timestamp for every operation                                     ║
║  • Success/Failed/Retry status for each message                      ║
║  • Detailed media group tracking                                     ║
║                                                                      ║
║  Log Files:                                                          ║
║  • clone_log.txt - Standard operational log                          ║
║  • clone_detailed_log.txt - Real-time detailed message tracking      ║
║  • clone_progress.json - Resumable progress data                     ║
║  • clone_stats.json - Statistics and metrics                         ║
║  • message_mapping.json - Source→Target ID mapping                   ║
║                                                                      ║
╚══════════════════════════════════════════════════════════════════════╝
    """)
    ...
    try:
        ...
        print("Check clone_log.txt and clone_detailed_log.txt for detailed error information.")
        print("\nCommon issues:")
