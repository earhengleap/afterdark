...
class Porn91Service:
            ...
            try:
                async with async_playwright() as p:
                    ...
                    try:
                        ...
                        # --- Priority 1: Decode strencode2 payload directly from the main page ---
                        ...
                        strencode_payload = await page.evaluate('''() => {
                            ...
                            if (playerContainer && playerContainer.parentElement) {
                                const scripts = playerContainer.parentElement.querySelectorAll('script');
                                for (const s of scripts) {
                                    const m = s.innerText.match(/strencode2\("([^"]+)"\)/);
                                    if (m) return m[1];
                                }
                            }
                            // Fallback: search all scripts for strencode2 with an mp4 URL
                        ...
                        if strencode_payload:
                            try:
                                import urllib.parse
                                decoded = urllib.parse.unquote(strencode_payload)
                                # Extract src='...' from the decoded <source src='...'> tag
                                src_match = re.search(r"src='(https?://[^']+)'", decoded)
                                if src_match:
                                    video_src = src_match.group(1)
                                    # Unescape HTML entities (e.g. &amp; -> &)
                                    video_src = video_src.replace('&amp;', '&')
                                    logger.info(f"Extracted video src via strencode2 decoding: {video_src[:60]}...")
                            except Exception as e:
                                logger.warning(f"strencode2 decode failed: {e}")

                        # --- Priority 2: VID-based embed mirror (fallback if strencode2 not found) ---
                        if not video_src:
                            vid = await page.evaluate('''() => {
                                const textarea = document.querySelector('#fm-video_link');
                                if (textarea && textarea.value) {
                                    const m = textarea.value.match(/VID=([a-zA-Z0-9]+)/);
                                    if (m) return m[1];
                                }
                                return null;
                            }''')
                            ...
                            if vid:
                                embed_url = f"https://91.9p9.xyz/ev.php?VID={vid}"
                                logger.info(f"strencode2 not found, falling back to VID embed: {embed_url}")
                                try:
                                    ...
                                    logger.warning(f"VID embed fallback failed: {e}")

                        # --- Priority 3: Generic CDN regex from page HTML ---
                        if not video_src:
                            logger.info("Falling back to generic regex extraction from main page")
                            html = await page.content()
                            # Search for generic cdn/mp4 patterns
                            m = re.search(r'(https?://[^\'"\s<>]+\.rsc\.cdn77\.org/mp43/[^\'"\s<>]+)', html)
                            if m:
                                video_src = m.group(1)
                            else:
                                # Last ditch: search for any viable mp4 source in the DOM
                                video_src = await page.evaluate('''() => {
                                    const video = document.querySelector('video#player_one_html5_api');
                                    if (video) return video.getAttribute('src') || video.querySelector('source')?.getAttribute('src');
                                    return null;
                                }''')
                        ...
                        if video_src:
                            logger.info(f"Successfully extracted target 91porn video src: {video_src[:50]}...")
                            return video_src, cookie_dict
                        
                        logger.warning(f"No video source identified for {url}")
    ...
    async def download_media(
        ...
        file_path = os.path.join(get_platform_folder("91Porn", "video"), filename)
