...
def get_following_list(username: str, cookies: Dict[str, str], resume: bool = True, progress_callback=None) -> Optional[List[str]]:
    ...
    try:
        # Get user ID
        msg = f"Looking up user: {username}"
        print(msg)
        ...
        if user_response.status_code != 200:
            msg = f"Error: Status {user_response.status_code}"
            print(msg)
            if progress_callback: progress_callback(msg)
            return None
        ...
        try:
            ...
            if is_protected:
                msg = f"\n🛑 Error: The account '@{username}' is Protected (Private)!\nYou cannot scrape their following list unless your cookie account is approved to follow them."
                print(msg)
                if progress_callback: progress_callback(msg)
                return None
        ...
        msg = f"Found {username}'s Internal ID: {user_id}"
        print(msg)
        if progress_callback: progress_callback(msg)
        ...
        if resume:
            ...
            if following_list and progress_callback:
                progress_callback(f"Resuming! Found {len(following_list)} users already saved.")
        ...
        msg = f"Fetching following list... (starting at page {page_num+1})"
        print(msg)
        if progress_callback: progress_callback(msg)
        ...
        while True:
                ...
                try:
                    ...
                    if response.status_code == 429:
                        ...
                        if reset_header:
                            ...
                            msg = f"Twitter Rate Limited! Sleeping for {wait_time} seconds (until {time.strftime('%H:%M:%S', time.localtime(reset_time))})..."
                            print(msg)
                            if progress_callback: progress_callback(msg)
                            time.sleep(wait_time)
                        else:
                            wait_time = 60 * (attempt + 1)
                            msg = f"Rate limited (No header info). Waiting {wait_time} seconds..."
                            print(msg)
                            if progress_callback: progress_callback(msg)
                            time.sleep(wait_time)
                        continue
                    ...
                    if response.status_code != 200:
                        msg = f"Error fetching following (page {page_num}): {response.status_code}"
                        print(msg)
                        if progress_callback: progress_callback(msg)
                    ...
                    if attempt < max_retries - 1:
                        msg = f"Request failed, retrying... ({attempt + 1}/{max_retries})"
                        print(msg)
                        if progress_callback: progress_callback(msg)
                        time.sleep(5)
                    else:
            ...
            try:
                ...
                msg = f"Page {page_num}: Found {users_found_this_page} new users (Total scraped: {count})"
                print(msg)
                if progress_callback: progress_callback(msg)
                ...
                if next_cursor is None or next_cursor == cursor or (not found_users and next_cursor):
                    msg = "Reached the end of the following list!"
                    print(msg)
                    if progress_callback: progress_callback(msg)
                ...
                msg = f"Error parsing response on page {page_num}: {e}"
                print(msg)
                if progress_callback: progress_callback(msg)
                break
            except Exception as e:
                msg = f"Unexpected error on page {page_num}: {e}"
                print(msg)
                if progress_callback: progress_callback(msg)
                break
